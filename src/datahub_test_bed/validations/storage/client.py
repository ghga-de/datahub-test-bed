...
class StorageClient:
    ...
    def list_all_object_in_bucket(
        self, bucket: str, prefix=None, return_objects=False, expect_error=False
    ):
    ...
    def create_multipart_upload(self, bucket: str, key: str, expect_error=False):
    ...
    def upload_part_copy(  # noqa: PLR0913
        ...
        try:
            ...
            return resp.get("CopyPartResult", {}).get("ETag", None)
        except botocore.exceptions.ClientError as error:
            error_message = get_error_message(error)
            log_unexpected_error(
            ...
            for i in range(PART_COUNT):
                ...
                with tempfile.NamedTemporaryFile("w+b") as temp_file:
                    ...
                    etag = self.upload_part(
                        bucket, key, part_num, temp_file.name, upload_id
                    )
                    if not etag:
                        logger.debug(
    ...
    def complete_multipart_upload(
        ...
        try:
            ...
            return bool(resp.get("Location", None))
        except botocore.exceptions.ClientError as error:
            error_message = get_error_message(error)
            log_unexpected_error(
    ...
    def upload_file_multipart(
        ...
        part_etags = self.split_file_and_upload_parts(bucket, file_path, key, upload_id)
        ...
        if expect_error:
            ...
            return
        logger.debug(
            'Uploaded %s parts to bucket "%s" using account "%s"',
            len(part_etags),
            bucket,
            self.profile_name,
        )
        if self.complete_multipart_upload(bucket, key, upload_id, part_etags):
            logger.info(
    ...
    def copy_file_multipart(
        ...
        if len(etags) != PART_COUNT:
            if expect_error:
                ...
                logger.debug(
                    'Failed to copy all file parts, expected "%s", got "%s"',
                    PART_COUNT,
                    len(etags),
                )
            return
        if expect_error:
            ...
            return
        logger.debug(
            'Copied %s parts to bucket "%s" using account "%s"',
            len(etags),
            dst_bucket,
            self.profile_name,
        )
        if self.complete_multipart_upload(dst_bucket, dst_key, upload_id, etags):
            logger.info(
    ...
    def get_presigned_url_for_object(self, bucket: str, key: str, expiration: int = 60):
        """Generate a presigned URL for an object."""
        if not self.head_object(bucket, key):
            return
        try:
    ...
    def delete_object(self, bucket: str, key: str, expect_error=False):
